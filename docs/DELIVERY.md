...
| 加载性能 | 懒加载 **0.01 s**、全量 eager **2.80 s**（隔离 3 轮中位）。早先的 5.5/11/35.8 s 都是 CPU 争用值 | 隔离重测 |
| 规模定律 | 信息集 ≈ **42 个/迭代** → 100K ≈ 420 万，**别直接上 1M**；要规模化先做状态抽象 | 三档实测外推 |
| **训练有效性** | **报胜率必须同报命中率**：10K 模型实战仅 **15~22%** 决策命中训练表，其余回落 RuleAgent —— 「MCCFR 胜率 55%」基本是 RuleAgent 打的 | `battle` 命中率报表 |
| 模型通用性 | **模型按玩家人数各训一份**：2 人模型进 3 人局命中率 **0%**（468 次决策 0 命中）且不报错 | 实测 + 现有警告 |
