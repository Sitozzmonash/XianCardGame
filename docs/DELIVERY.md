...
> 更新日期：2026-09-12（§3.3/§6 的前端部分已被 2026-09-13 的重做取代，见下方提示）

> ⚠️ **前端已于 2026-09-13 整体重做**：按用户提供的参考原型（Next.js 原型，留档 `docs/reference-next/`）
> 1:1 移植，旧前端已被整棵替换（删除 82 个旧文件）。**本文档 §3.3 / §6 里关于前端的描述与截图已过时**，
> 前端相关的现状、还原度实测、卡牌交互矩阵与验证证据请以 **`docs/FRONTEND_PORT_REPORT.md`** 为准
> （还原度：与参考基准图逐屏像素差 9.1–17.4，旧版同口径 80.3）。
> 卡牌规则也按原型文案做了有意偏离（观星术 +改序 / 遁术 → 反应牌 / 反制符 → 反弹），
> 详见 `docs/CARD_RULES_DELTA.md` 与 `docs/INTERFACES.md` 附录 A13。
...
| 截图存证 | `docs/screenshots/real_10_result.png`（本局真实结算页，2026-09-13 重做后重采） |
