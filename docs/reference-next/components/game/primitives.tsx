      ...
      {options.map((option) => {
        ...
        return (
          <button
            ...
            onClick={() => onChange(option)}
            className={cn(
              'rounded-lg border px-2.5 py-1 font-sans text-[11px] font-medium tracking-wide transition-all duration-200',
              active
    ...
    <header className={cn('relative z-10 px-5 pt-6', className)}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          {eyebrow && (
            <p className="font-sans text-[11px] font-medium tracking-[0.3em] text-jade-300/80">{eyebrow}</p>
          )}
