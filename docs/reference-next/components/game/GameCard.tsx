...
export function GameCard({
  ...
  return (
    ...
    >
      ...
      {typeof index === 'number' && (
        <span className="absolute left-1.5 top-1.5 flex h-5 w-5 items-center justify-center rounded-md border border-gold-400/60 bg-ink-950/80 font-sans text-[10px] font-semibold text-gold-300">
          {index}
        </span>
      )}
      ...
      <div className="absolute inset-x-0 bottom-0 px-2 pb-2 pt-6">
        ...
        {size !== 'sm' && (
          <p className="mt-0.5 truncate font-sans text-[10px] tracking-wide text-jade-300/80">{card.subtitle}</p>
        )}
      </div>
    </button>
  )
