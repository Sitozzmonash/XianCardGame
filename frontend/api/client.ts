 ...
 *
 * base URL：NEXT_PUBLIC_API_BASE_URL，缺省 http://localhost:8000/api/v1
 * mock 开关：NEXT_PUBLIC_USE_MOCK（'1'/'true'/'yes' → 使用 api/mock.ts；缺省 = 走真后端）
 *
 * 注意：Next.js 只在构建期静态替换 `process.env.NEXT_PUBLIC_*` 字面量，
...
/** 构建期开关：**只有**显式设置 NEXT_PUBLIC_USE_MOCK=1/true/yes 才走 mock；默认走真后端 */
