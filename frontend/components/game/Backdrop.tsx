      ...
      <svg viewBox="0 0 400 400" className="h-full w-full">
        ...
        <g className={spin ? 'animate-spin-slow' : undefined} style={{ transformOrigin: '200px 200px' }}>
          ...
          {Array.from({ length: 24 }).map((_, i) => {
            ...
            // 固定小数位，避免 Node/Chromium 浮点序列化差异造成 hydration warning。
            const x1 = Number((200 + Math.cos(angle) * 150).toFixed(4))
            const y1 = Number((200 + Math.sin(angle) * 150).toFixed(4))
            const x2 = Number((200 + Math.cos(angle) * 168).toFixed(4))
            const y2 = Number((200 + Math.sin(angle) * 168).toFixed(4))
        ...
        <g className={spin ? 'animate-spin-slower' : undefined} style={{ transformOrigin: '200px 200px' }}>
          ...
          {Array.from({ length: 8 }).map((_, i) => {
            ...
            const x1 = Number((200 + Math.cos(angle) * 112).toFixed(4))
            const y1 = Number((200 + Math.sin(angle) * 112).toFixed(4))
            const x2 = Number((200 + Math.cos(angle) * 128).toFixed(4))
            const y2 = Number((200 + Math.sin(angle) * 128).toFixed(4))
