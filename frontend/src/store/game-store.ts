...
export const DEFAULT_SETUP: SetupConfig = {
  ...
  mccfrModel: 'models/mccfr_2p_100k.pkl',
  seed: '',
};
...
function buildAgentSpec(setup: SetupConfig, seat: number): AgentSpec {
  const type = setup.agentTypes[seat] ?? 'rule';
  switch (type) {
    case 'ismcts':
      return { type: 'ismcts', simulations: setup.ismctsSimulations };
    case 'mccfr':
      return { type: 'mccfr', model: setup.mccfrModel };
    case 'random':
      return { type: 'random' };
    default:
      return { type: 'rule' };
...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  disableDemoMode() {
    ...
    set({
      ...
      isRefreshing: false,
      selectedCardId: undefined,
      notice: '已切回真后端模式，请确认 backend 已在 EXPO_PUBLIC_API_BASE_URL 上运行。',
    });
  },
